...
| Job | Cron | O que faz |
|-----|------|-----------|
| `fetch-results` | `*/5 * * * *` | Puxa placares da FIFA API/scraping, atualiza `Jogo`; no mata-mata captura `classificadoLado`/`decididoNosPenaltis` e, ao fim do tick, roda o `advance-bracket` |
...
### 23.4 Seed (`scripts/seed-mata-mata.ts`, `npm run seed:mata-mata`)
- Bloco `DATA_R32` editável (16 linhas `num | Casa x Visitante | data | hora | Sede`) — o dono troca
  pelos confrontos reais **sem mexer em código**. Horário é local da sede → convertido pra UTC
  (`horaLocalSedeParaUtc`). Cria as 6 rodadas por bolão ATIVO (transacional), R32 com times reais e
  oitavas→final com placeholder + ligações da chave. **Idempotente**, re-rodável.
- **Portão**: R32 só vira ABERTA quando os 16 jogos têm dois times reais. `--dry-run` valida
  transcrição + fuso sem gravar (import do DB é dinâmico → não exige DATABASE_URL).
