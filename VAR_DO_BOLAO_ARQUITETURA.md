...
**Última atualização:** 2026-06-11 (v3.22.0 — Placar AO VIVO via `api.fifa.com` + provider `hybrid` [FIFA primário → openfootball fallback]. IDs reais `idCompetition=17`/`idSeason=285023`, status codes corrigidos [0=FINALIZADO, 1=AGENDADO, 3=AO_VIVO], match por par de código FIFA. Trava de pontuação: só FINALIZADO pontua. `handlePlacarJogo` mostra placar parcial ao vivo. 795 tests)
