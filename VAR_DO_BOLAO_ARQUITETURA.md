...
**Versão do documento:** 3.20.0
**Última atualização:** 2026-06-11 (Jogo em andamento — análise feita com México x África ROLANDO: status AO_VIVO nunca existe no banco durante o jogo [openfootball não dá placar ao vivo], então "qual o placar?" respondia "não achei jogo rolando" com jogo rolando. Novo helper `jogo-status.ts` deriva "rolando" por HORÁRIO [kickoff + janela 2.5h]. PLACAR_JOGO/STATUS_RODADA/PROXIMOS_JOGOS mostram 🔴 ROLANDO/⏳ aguardando placar. Preview de palpite avisa jogos já iniciados ANTES do "sim")
