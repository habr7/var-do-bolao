...
**Última atualização:** 2026-07-02 (v3.60.0 — **Histórico de conversas + auditoria de palpite**: toda mensagem (entrada/saída, ambos os canais) fica em `mensagens_conversa` (hooks fire-and-forget nos 2 chokepoints: `handleIncomingMessage` e `sendText`/`sendImage` — logging nunca derruba conversa; pulado em dry-run). Trilha `palpites_auditoria`: cada REGISTRADO/EDITADO/APAGADO/CLASSIFICADO vira evento imutável com placar antes→depois, o TEXTO CRU da mensagem que originou (via `auditoria-contexto`, ponte em memória router→service) e o canal — prova direta pra disputa. Comandos de dono: `#CONVERSAS <número|nome> [N]`, `#CONVERSASGLOBAL [N]`, `#AUDITORIA <número|nome> [N]` (interceptados no topo, padrão broadcast; busca por nome fuzzy ou número com variantes de 9º dígito). Retenção LGPD: `CONVERSA_RETENCAO_DIAS` (default 180) no job mensal; auditoria NÃO expira. Migration aditiva. +22 testes; 1245 tests, simulador 143. Ver seção 25. v3.59.0 — **Canal TELEGRAM (multi-canal)**: o bot agora fala WhatsApp E Telegram com a MESMA lógica — novo diretório `src/messaging/` (telegram.client/format/identity/inbound/transport + channel-router), tudo na fronteira de I/O: o `command.router` continua 100% agnóstico de canal. Entrada: polling (getUpdates, default, zero infra pública) OU webhook (`/webhook/telegram`, header secreto), por `TELEGRAM_MODE`. Saída: `sendText/sendImage` roteiam por destinatário via `channel-router` (cache 60s; dialeto WhatsApp→HTML com fallback texto puro; quebra >4096; retry 429). Identidade: onboarding no 1º contato pede o número de WhatsApp, casa variantes (9º dígito, sufixo JID) e vincula `telegramId` ao Usuario existente → pontuação/palpites recuperados sem tocar no router. Schema: +`telegramId`/`telegramUsername`/`canalPreferido` no Usuario (migration aditiva). Flags: `ENABLE_WHATSAPP` (default true) / `ENABLE_TELEGRAM` (default false) — defaults preservam comportamento atual; migração = inverter as duas. Convite ciente de canal (deep link t.me quando WhatsApp off). Ver seção 24. +44 testes; 1223 tests, simulador 143. v3.58.0 — Fix: usuário COPIA a lista "Próximos jogos" do bot e põe placares (caso Rafael 01/07) agora é entendido. (A) parser remove markdown `_`/`*` antes de parsear → o sufixo `_(16-avos de final)_` deixa de poluir o nome do time E de estourar o guard de 40 chars (a "Bósnia e Herzegovina" não é mais descartada). (B) `PERGUNTA_GERAL_FUTEBOL` entra no lote-override → 2+ palpites vencem a "pergunta" que os rótulos de fase acionam → PALPITE_INLINE (preview+confirma), sem cair no LLM. +6 testes; 1179 tests, simulador 143. v3.57.0 — "Meus palpites" FILTRADO por jogo/fase/dia: "qual meu palpite no França x Suécia" mostra SÓ esse jogo (+ comparação oficial), direto, sem "quer ver todos? sim/não". Também "meus palpites do mata-mata/das oitavas/de hoje". Reusa `montarStatusResultado`+`linhaClassificadoMeusPalpites`; detecção de time via `construirFatosCopa2026`, fase/dia por keyword; varre todos os bolões (rotula por bolão); sem filtro → resumo de sempre. Override no parser pra "meus palpites <fase>" não ser sequestrado por info-intent. +8 testes. 1165 tests, simulador 143. Sem migration. v3.56.0 — BUG CRÍTICO: `ENABLE_*=false` no `.env` não desligava — os flags usavam `z.coerce.boolean()` e `Boolean("false")===true`, então `ENABLE_LEMBRETE_30MIN=false` virava TRUE e o lembrete disparava (idem bom-dia/ranking/reveal). Trocado por `z.preprocess(coerceBool, z.boolean())`; +3 testes. 1165 tests. v3.55.0 — Script admin `scripts/inserir-palpite-manual.ts`: insere palpite legítimo perdido (bot fora do ar + comprovação por print) SEM a trava de "jogo já começou". Telefones por ARGUMENTO (não vão pro repo). Cria/atualiza Palpite+PalpiteJogo em todos os bolões do usuário com aquele jogo, alinha o placar à ordem oficial casa/visitante e recalcula na hora. Tem `--dry-run`. Sem mudança no app. v3.54.0 — "Palpite da galera" mais limpo + on-demand como caminho principal (sem revelação automática): (1) `PALPITE_OUTROS` ganhou default de escopo = só os jogos **ROLANDO** (mais limpo); no fim oferece "manda *palpites de todos*" pra ver também os finalizados de hoje (breadcrumb 5min faz um "todos" seco funcionar). Citar time OU pedir "todos" → escopo total; sem jogo rolando → cai nos finalizados recentes (não deixa no vácuo). `revelacoesParaUsuario` ganhou param `escopo` ('rolando'|'todos', 1 query só devolvendo os dois totais). (2) +frases no parser (chutes/apostas da galera, "como a galera foi", "palpites do pessoal/turma/rapaziada", "placares dos outros participantes") sem sequestrar `MEU_PALPITE`. +15 testes. 1162 tests. Sem migration. v3.53.0 — Controle ANTI-BAN dos disparos automáticos (incidente real 30/06: número do bot suspenso por spam de automação): (1) `send-ranking` (mandava posição pra CADA participante por rodada, de hora em hora) NÃO tinha flag — agora tem `ENABLE_RANKING` (OFF por padrão). (2) defaults SEGUROS: TODOS os disparos em massa (`ENABLE_BOM_DIA`, `ENABLE_PALPITE_REVEAL`, `ENABLE_LEMBRETE_30MIN`) viram OFF por padrão — um deploy nunca blasta sozinho num número recém-recuperado; liga-se explicitamente no `.env`. (3) a revelação de palpites no kickoff fica SÓ sob demanda ("palpite dos participantes" → PALPITE_OUTROS); o bom-dia ganhou uma nota avisando disso. +3 testes. 1147 tests. Sem migration. v3.52.0 — Refino da blindagem v3.51.0 após bateria de ~45 formatos: (1) o detector `pareceTentativaDePalpite` parou de excluir `ganha/ganho/perde/perco` — eram justamente as palavras do palpite NL ("Brasil ganha de 2 a 1", "Argentina perde por 1 a 0"), que caíam fora da rede; as perguntas tipo "quanto ganho" seguem barradas por "quanto/quantos"; teto 60→70. (2) o regex vírgula/"e" passou a exigir gols ≤12 (placar real) — sem isso, enumeração de datas/números ("o Brasil joga 15 e a Argentina 16") virava palpite absurdo "15x16"; agora cai pro LLM/conversa. Garantido por teste: NENHUM formato (palpite ou não) chega a "registrei/Combinado" sem preview + confirmação + match de time real. +8 testes. 1144 tests. Sem migration. v3.51.0 — BLINDAGEM robusta de palpite (continuação do caso 29/06): (1) +2 formatos de placar por regex — `Time1 N, Time2 M` e `Time1 N e Time2 M` (vírgula/"e" com gol depois do time; vírgula exige dígito antes pra não quebrar lista real). (2) **Rede de segurança via LLM**: novo `pareceTentativaDePalpite` (heurística ampla — 2+ números de placar + 2+ nomes prováveis de time, não-pergunta) dispara o EXTRATOR LLM contra os jogos OFICIAIS **mesmo fora da janela de "próximos jogos"** → qualquer formato inesperado cai no preview + confirmação canônico (nunca registra direto, só age se casar times reais). Fecha a classe inteira do bug em vez de caçar regex infinito. +14 testes, simulador 143. 1136 tests. Sem migration. v3.50.0 — Fix GRAVE de parse de palpite: formato `Time1 N x Time2 N` (gol DEPOIS de cada time, ex. "Alemanha 2 x Paraguai 3") caía em TEXTO_LIVRE e NÃO registrava; o "Sim" seguinte virava "🙌 Combinado!" (concordância casual), dando falsa impressão de palpite anotado. Novo `PALPITE_GOLS_POS_TIME_REGEX` (5º formato de placar, roda por ÚLTIMO com guards anti-lixo) → agora vai pro preview + confirmação canônico. +5 testes unit, +1 no simulador (142). 1122 tests. Sem migration. v3.49.0 — Fix do "bom dia" DUPLICADO: pra quem está em 2+ bolões, o aviso diário listava cada confronto uma vez POR bolão (mesma rodada R32 replicada por bolão) — agora `send-bom-dia.job` deduplica por confronto (`apiJogoId`) e só marca ✅ se a pessoa palpitou em TODOS os bolões com aquele jogo (senão ⚪, pra não esconder pendência). NÃO havia duplicação no banco (palpites intactos). Prevenção: `seed-mata-mata.ts` passa a rodar o sync sob o lock `fetch-results` (serializa com o tick automático, fecha a corrida que poderia criar rodada duplicada). 1117 tests. Sem migration. v3.44.0 — Robustez de conversa do mata-mata: classificado no MESMO palpite do empate ["Brasil 1x1 Japão e o Brasil passa" → preview mostra placar + quem passa, confirma antes de registrar; `PalpiteInline.classificado`, corrige inversão de lado], guard pra palpite real não virar intent de dúvida ["…nos pênaltis" não perde mais o palpite], roteamento da bateria de QA ["jogos de hoje"→JOGOS_HOJE, "tabela"→RANKING, conjugações/typos], exceção do mata-mata na REGRA-CHAVE do classificador LLM, escape gracioso do `CONFIRMANDO_CLASSIFICADO_MATAMATA`. Regras/KNOWLEDGE_PRODUTO atualizados. 1101 tests, simulador 141/141. v3.41.0 — Fase MATA-MATA da Copa 2026 [16-avos→final+3º]: migration aditiva [FaseTorneio/LadoJogo + colunas em Rodada/Jogo/PalpiteJogo], pontuação por fase + bônus de classificado [placar 90'+prorrogação, pênalti fora; crava nunca perdida], chave determinística `bracket-2026.ts`, seed `seed-mata-mata.ts` [dados à noite só preenchendo o bloco, idempotente, --dry-run], `advance-bracket.job`, FSM de empate `CONFIRMANDO_CLASSIFICADO_MATAMATA` + submenu de regras + 11 intents de dúvida/chave. Ver seção 23. 1071 tests, simulador 139/139. v3.37.1 — Palpites com separador × (unicode) e "c" (typo de x) agora parseiam; palpite incompleto "Espanha 4x1" pede o adversário. v3.37.0 — Fix grave: placar de Costa do Marfim x Equador (e Irã/RD Congo) nunca atualizava — o `nome` no teams.json divergia dos fixtures/DB, quebrando o matcher da FIFA (fifaCode→nome→fixture) e o grounding; nomes alinhados + teste de consistência; auto-corrige no próximo tick. v3.36.0 — "Bom dia boleiros" em hora fixa (9h BRT) pra TODOS de uma vez (antes era kickoff-6h e só parte recebia); chamada de palpites desativada (redundante). v3.35.0 — Fix GRAVE: lista rotulada "Meus palpites:\n<jogos>" virava MEU_PALPITE e era ignorada; agora lote de 2+ palpites vence intent de leitura, e prefixo de data/hora do formato do bot é removido. v3.34.0 — Fix GRAVE: palpites separados por VÍRGULA não registravam (caso Felipe) — parser só dividia por \n; agora aceita vírgula/; (formato que o bot anuncia). v3.33.0 — Fix GRAVE de display: "meus palpites" mostrava jogo AO VIVO como final com "0 pts ❌" (placar parcial da FIFA tratado como oficial); render agora decide pelo status. v3.32.0 — Estrutural: whitelist do classificador LLM completada (+teste anti-drift), [DADOS AO VIVO] no smart-fallback (LLM responde placar/rolando com dado real), patterns "rolando agora", e revisão diária automática das não-entendidas pro dono. v3.31.0 — Lembrete por JOGO ~30min antes do kickoff pra quem não palpitou (com idempotência por jogo + cooldown + cap); send-reminders por-rodada desativado por redundância. v3.30.0 — Fix "sair do bolão" pra quem é admin de um e participante de outro: agora explica por que o bolão-admin não aparece + aceita "sair do bolão X" inline. v3.29.0 — Fix matching de abreviação de times: "Coreia 1x0 Rep Checa" agora acha "Coreia do Sul x República Tcheca" [alias + token-match + fallback LLM restrito que só traduz nomes]. v3.28.0 — Auditoria do código, Tier 1+2: paginação do "meus palpites"; criação de bolão sem senha; cap de avisos atômico + lock nos jobs de pontuação; recalcularRanking sem N+1; índices em hot paths; revelação avisa quando corta. v3.27.0 — UX pós-início da Copa: "meus palpites" agrupado por data; "próximos jogos" pergunta filtro [só pendentes/todos] com novo estado FSM; perguntas de placar/finalizados casam PLACAR_JOGO em vez de cair na LLM; "placares dos demais" revela palpites de jogo já iniciado sem janela de 24h. v3.26.2 — Fix ranking sob demanda fora de ordem em empate (caso "1,2,3,5,4"): ordenação determinística por cascata + posição derivada do índice. v3.26.1 — Copy de latência atualizada pro placar AO VIVO da FIFA: removidas as mensagens antigas de "placar oficial em ~1h após o apito" / "placar ao vivo não existe"; agora dizem placar ao vivo + pontos poucos min após o apito final. v3.26.0 — Broadcast administrativo: dono manda `#ENVIOPARAVARDOBOLAO# msg` no WhatsApp e dispara pra todos os usuários [modo-teste envia só pro dono]; interceptado no topo do pipeline, idempotente e com throttle. v3.25.0 — Palpite com times INVERTIDOS [ex: "República Tcheca 2x0 Coreia do Sul" pro jogo "Coreia x Tcheca"] agora é entendido: matching tolera ordem trocada e arruma nomes+placar na confirmação. v3.24.0 — Revelação de palpites no kickoff: quando o jogo começa, manda pros integrantes os palpites de todos do bolão pra aquele jogo [push job time-driven + resposta sob demanda no PALPITE_OUTROS]. Privacidade vira TEMPORAL [privado antes, revelado depois]. `MAX_AVISOS_DIA` 2→8. v3.23.0 — Janela de polling [API só na janela ativa do jogo]. v3.22.0 — Placar AO VIVO via `api.fifa.com` + provider `hybrid`. 806 tests)
...
│   │
│   ├── messaging/                   # (v3.59.0) Canal Telegram + roteador multi-canal
│   │   ├── channel-router.ts        # Decide WhatsApp × Telegram por destinatário (cache 60s)
│   │   ├── telegram.client.ts       # Cliente Bot API (sendMessage/sendPhoto/getUpdates/setWebhook)
│   │   ├── telegram.format.ts       # Dialeto WhatsApp → HTML Telegram + quebra >4096
│   │   ├── telegram.identity.ts     # Vínculo telegramId↔Usuario + normalização de número BR
│   │   ├── telegram.inbound.ts      # Update → onboarding OU handleIncomingMessage
│   │   └── telegram.transport.ts    # Polling (getUpdates) e webhook (/webhook/telegram)
│   │
│   ├── whatsapp/
│   │   ├── webhook.handler.ts       # GET + POST /webhook/whatsapp (Evolution v2)
│   │   ├── admin-conversas.ts       # (v3.60.0) Dono: #CONVERSAS / #CONVERSASGLOBAL / #AUDITORIA
│   │   ├── evolution.client.ts      # Ponto único de envio: roteia WhatsApp × Telegram (sendText, markAsRead)
...
│   │   ├── conversa/                # (v3.60.0) Histórico de conversas + auditoria de palpite
│   │   │   ├── conversa.service.ts  # registrar/consultar mensagens + eventos (fire-and-forget)
│   │   │   └── auditoria-contexto.ts # ponte em memória router→service (texto cru + canal)
│   │   ├── bolao/
│   │   │   ├── bolao.types.ts
...
| `ESTATISTICA_PONTOS` (v3.38.0) | "quantas cravadas eu fiz?", "quantos placares exatos acertei?", "quantos fiz 10/7/5/3 pontos?", "quantas vezes zerei?", "estatística/resumo dos meus pontos", "de onde vêm meus pontos", "meu aproveitamento" | Read-only: conta os `pontosObtidos` por FAIXA (10/7/5/3/0) sobre palpites JÁ calculados (jogo `FINALIZADO` + `Palpite.calculado=true`) via `getEstatisticaPontos`. Mostra a quebra completa + total + posição; se o user perguntou uma faixa específica, prefixa um destaque (ex: "🎯 Você cravou *2 vezes*!"). Rodapé cross-linka pro drill-down (*quais cravei*). Resolve o bolão espelhando o RANKING (padrão → 1 direto → estado `ESCOLHENDO_BOLAO_ESTATISTICA`). Faixas vêm de `PONTUACAO_PADRAO`. Precedência ANTES de PONTOS_DETALHE e MEUS_PONTOS (patterns exigem menção a faixa/estatística, então "quantos pontos fiz ontem"→PONTOS_DETALHE e "meus pontos"→MEUS_PONTOS seguem intactos). |
| `JOGOS_POR_FAIXA` (v3.39.0) | "quais jogos eu cravei?", "quais cravei", "me mostra as cravadas", "quais jogos fiz 7 pontos?", "quais deram 5", "me mostra os de 3 pontos", "quais jogos eu zerei?", "em quais errei tudo" | Read-only: drill-down da estatística — LISTA os jogos do user de UMA faixa (10/7/5/3/0) com palpite + resultado real, via `getJogosPorFaixa` (reusa `buscarPalpitesJogosPorFaixa` + `getEstatisticaPontos` pra a régua de faixas no rodapé). Faixa detectada por `detectarFaixaEstatistica`; se ambígua ("me mostra meus jogos por pontuação"), cai no resumo agregado. Reusa o estado `ESCOLHENDO_BOLAO_ESTATISTICA` (ctx `estatisticaListarJogos`) pro multi-bolão. **Precedência ANTES de ESTATISTICA_PONTOS** — patterns exigem gatilho de LISTAGEM (quais/me mostra/ver) + faixa, então "quantas cravadas" (contagem)→ESTATISTICA_PONTOS e "quais cravei" (lista)→JOGOS_POR_FAIXA. |
...
model Usuario {
  ...
  telegramId       String? @unique        // (v3.59.0) chat.id do Telegram, setado no onboarding
  telegramUsername String?                // (v3.59.0) @username (informativo)
  canalPreferido   String?                // (v3.59.0) 'whatsapp' | 'telegram' | null
...
**Mata-mata (Copa 2026):** os valores de placar sobem por fase e existe um bônus
de classificado aditivo. Ver a seção **23. Mata-mata** — `TABELA_PONTOS` e
`BONUS_CLASSIFICADO` (`ranking.types.ts`) são a fonte única; `GRUPOS` e `R32`
referenciam `PONTUACAO_PADRAO` (zero regressão na fase de grupos).
...
| Job | Cron | O que faz |
|-----|------|-----------|
| `fetch-results` | `*/5 * * * *` | Puxa placares da FIFA, atualiza `Jogo` (grupos casam por código de país); no fim do tick roda o **mata-mata-sync** (times/datas/placar/classificado de TODAS as fases via `MatchNumber`) + recalcula + `advance-bracket` (fallback) |
| `advance-bracket` | (dentro do fetch-results) | Mata-mata: ao finalizar um jogo com classificado, grava o vencedor no slot do próximo jogo (e o perdedor das semis no 3º lugar); abre a rodada quando os dois lados ficam reais. Idempotente (só escreve slot placeholder). `advanceBracketComClient(db)` é o núcleo testável. |
| `calculate-scores` | `*/10 * * * *` | Calcula pontos de rodadas finalizadas (ramifica por `rodada.fase`: grupos → `calcularPontos`; mata-mata → `pontuarJogoMataMata`) |
| `send-reminders` | `*/30 * * * *` | Cutuca quem tem `jogosPendentes` |
| `send-palpite-reveal` | `*/2 * * * *` | No kickoff, revela pros integrantes os palpites de todos do bolão pra aquele jogo. Time-driven, idempotente por `(user, jogo)`, conta no `MAX_AVISOS_DIA`. **`ENABLE_PALPITE_REVEAL` (OFF por padrão desde v3.53.0)** — revelação fica sob demanda (`PALPITE_OUTROS`). |
| `send-ranking` | `0 * * * *` | Posição no ranking pra CADA participante após rodada finalizar (disparo em massa). **`ENABLE_RANKING` (OFF por padrão desde v3.53.0)**, idempotente por `(rodada, user)`. |
| `send-bom-dia` | `0 * * * *` | Saudação nos dias com jogo (decide horário internamente). 1 msg por telefone, **deduplicada por confronto** (`apiJogoId`) pra quem está em 2+ bolões; ✅ só se palpitou em TODOS os bolões com o jogo. |
...
# Canais de mensageria (v3.59.0 — multi-canal WhatsApp + Telegram)
ENABLE_WHATSAPP=true                 # canal WhatsApp (Evolution) ligado
ENABLE_TELEGRAM=false                # canal Telegram ligado (exige TELEGRAM_BOT_TOKEN)
TELEGRAM_BOT_TOKEN=                  # token do @BotFather (123456:ABC-DEF...)
TELEGRAM_BOT_USERNAME=               # @username do bot SEM @ (pro deep link t.me)
TELEGRAM_MODE=polling                # polling (default, zero infra) | webhook (exige APP_URL público HTTPS)
TELEGRAM_WEBHOOK_SECRET=             # só no modo webhook: segredo do header X-Telegram-Bot-Api-Secret-Token

# Histórico de conversas (v3.60.0) — retenção LGPD em dias (job mensal).
# A trilha de auditoria de palpite (palpites_auditoria) NÃO expira.
CONVERSA_RETENCAO_DIAS=180
```
...
|--------|------|---------------------|
| **3.58.0** | **2026-07-01** | **Fix: COPIAR a lista "Próximos jogos" do bot + pôr placares agora registra (caso real Rafael 01/07).** O usuário copiou a lista que o próprio bot mandou (com bullets `⚪ data, hora —` e o rótulo de fase em markdown `_(16-avos de final)_`) e adicionou os placares nas 10 linhas. O bot **ignorou** e respondeu "fiquei sem fôlego" (o LLM conversacional estava congestionado). Duas causas: **(A) parsing** — o parser não removia o markdown, então o sufixo `_(16-avos de final)_` **poluía o nome do time** ("Áustria _(16-avos de final)_") e a "Bósnia e Herzegovina" **estourava o guard de 40 chars** e era descartada (9/10 parseados, sujos). Fix: `tentarParsearPalpiteInline` remove `_`/`*` antes de parsear (nome de time nunca tem markdown; o `(fase)` vira rabicho ignorado). **(B) roteamento** — os rótulos de fase acionavam `PERGUNTA_GERAL_FUTEBOL` (padrão `oitavas|quartas|...`), que **não** estava no lote-override → ia pro LLM em vez de virar palpite. Fix: `PERGUNTA_GERAL_FUTEBOL` entra em `INTENTS_LEITURA_SOBRESCRITAS_POR_LOTE` (só sobrescreve com 2+ palpites parseáveis; pergunta pura de fase segue intacta). Agora a lista copiada → `PALPITE_INLINE` → preview + confirmação dos 10, **sem depender do LLM**. **+6 testes** (o texto exato do Rafael, Bósnia limpa/presente, anti-regressão de pergunta de fase). Bateria ampla extra: **1179 tests**, simulador 143. Sem migration. |
| **3.57.0** | **2026-07-01** | **"Meus palpites" FILTRADO por jogo / fase / dia.** Antes, "qual meu palpite no França x Suécia?" ignorava o jogo citado, mostrava o resumo e (no "sim") despejava a Copa inteira. Agora `MEU_PALPITE` detecta um filtro no texto e mostra **só** o que casa, **direto** (sem "quer ver todos? sim/não"): **(1) por jogo/time** ("qual meu palpite no França x Suécia", "meu palpite da França") → aquele(s) jogo(s) + comparação oficial, varrendo **todos os bolões** do usuário (rotula por bolão se >1). **(2) por fase** ("meus palpites do mata-mata / das oitavas"). **(3) por dia** ("meus palpites de hoje / de ontem"). Reusa o render (`montarStatusResultado` + `linhaClassificadoMeusPalpites`, extraído pra `renderBlocosPalpites`); detecção de time via `construirFatosCopa2026`, fase/dia por keyword. **Sem filtro → resumo + sim/não; filtro sem match → cai no resumo (nunca "não encontrei").** No parser, override pra "meus palpites <fase>" não ser sequestrado por info-intent (`INTENTS_CEDEM_PRA_MEUS_PALPITES`), sem pegar reclamação. **+8 testes.** **1165 tests**, simulador 143. Sem migration. |
| **3.56.0** | **2026-07-01** | **BUG CRÍTICO de config: `ENABLE_*=false` no `.env` NÃO desligava (incidente real 01/07).** O lembrete de 30min disparou **mesmo com `ENABLE_LEMBRETE_30MIN=false`** no `.env`. Causa: os 7 flags `ENABLE_*` usavam **`z.coerce.boolean()`**, e `Boolean("false") === true` — ou seja, a **string** `"false"` era coagida pra **`true`**. Então TODO `ENABLE_*=false` (bom-dia, ranking, reveal, lembrete) estava na prática **LIGADO** → o "controle anti-ban" da v3.53.0 não funcionava quando setado explicitamente (só a AUSÊNCIA do flag, via `.default(false)`, desligava). Fix: troca por **`z.preprocess(coerceBool, z.boolean())`** (o `coerceBool` já existente trata `"false"/"0"/"" → false` e `"true"/"1" → true`). **+3 testes de regressão** travando a coerção. **1165 tests.** Sem migration. Guardado também `docs/plano-palpite-unico.md` (filtro de "meus palpites" por jogo/seleção, pra implementar depois). |
| **3.55.0** | **2026-07-01** | **Script admin `scripts/inserir-palpite-manual.ts`** — insere/altera palpite legítimo perdido (bot fora do ar + comprovação por print) SEM a trava de "jogo já começou". Telefones por argumento (não vão pro repo); acha usuário e jogo por nome normalizado (distingue Suécia de Suíça), alinha o placar à ordem oficial e recalcula. Tem `--dry-run`. |
| **3.54.0** | **2026-07-01** | **"Palpite da galera" mais limpo + on-demand vira o caminho principal.** Com a revelação automática no kickoff desligada (v3.53.0), ver o que a galera cravou passa a ser 100% sob demanda — então o fluxo foi lapidado. **(1)** `PALPITE_OUTROS` agora tem **escopo default = só jogos ROLANDO** (antes despejava live + finalizados das últimas 24h). No rodapé oferece **"manda *palpites de todos*"** pra incluir os finalizados de hoje; um **"todos"/"tudo" seco** logo depois também funciona (breadcrumb Redis 5min). Citar um time ("palpites de México") **ou** pedir "todos" força escopo total; se **não houver nada rolando**, cai automaticamente nos finalizados recentes (não deixa o usuário no vácuo). `revelacoesParaUsuario` ganhou o parâmetro `escopo: 'rolando'\|'todos'` resolvendo tudo em **1 query** (devolve `totalRolando`/`totalTodos` pro caller decidir o rodapé). **(2)** Parser: +frases que faltavam pra `PALPITE_OUTROS` — "chutes/apostas da galera", "como a galera foi", "palpites do pessoal/turma/rapaziada", "placares dos outros participantes", "ver os palpites do pessoal" — com anti-regressão garantindo que **"meus palpites" continua `MEU_PALPITE`**. **+15 testes.** **1162 tests.** Sem migration. |
| **3.53.0** | **2026-06-30** | **Controle ANTI-BAN dos disparos automáticos (incidente real 30/06).** O número do bot foi **suspenso por spam** (automação não-oficial + envios em massa). Auditoria dos jobs revelou que **`send-ranking`** (posição no ranking pra CADA participante, de hora em hora após rodada finalizar) **não tinha interruptor** — era um disparo em massa sem flag. Correções: **(1)** novo `ENABLE_RANKING` (OFF por padrão) gateando o `send-ranking`. **(2) Defaults SEGUROS** — `ENABLE_BOM_DIA`, `ENABLE_PALPITE_REVEAL` e `ENABLE_LEMBRETE_30MIN` agora são **`false` por padrão**: um deploy num número recém-recuperado **nunca** dispara em massa sozinho; liga-se explicitamente no `.env` só o que quiser. **(3)** a revelação de palpites no kickoff fica **só sob demanda** (intent `PALPITE_OUTROS`, gatilho "palpite dos participantes"); o **bom-dia** ganhou uma nota curta avisando disso ("👀 manda *palpite dos participantes* pra ver o que a galera cravou"). **+3 testes** (gate do ranking + nota do bom-dia). **1147 tests.** Sem migration. |
| **3.52.0** | **2026-06-29** | **Refino da blindagem v3.51.0 após bateria de ~45 formatos (test-first).** Rodando 10+ formatos pra "ver como o LLM responderia" achou 2 defeitos na própria v3.51.0: **(1)** o detector `pareceTentativaDePalpite` excluía `ganha/ganho/perde/perco` — mas essas SÃO as palavras do palpite em linguagem natural ("Brasil **ganha** de 2 a 1", "Argentina **perde** por 1 a 0"), que ficavam fora da rede de segurança e caíam num "pede adversário" com lixo. Removidas das exclusões (perguntas "quanto **ganho**" seguem barradas por `quanto/quantos`); adicionadas como stopword de time; teto 60→70 chars. **(2)** o regex vírgula/"e" não limitava os gols → uma enumeração de datas/números ("o Brasil joga **15 e** a Argentina **16**") virava um palpite absurdo `15x16` no preview. Agora exige **gols ≤12** (placar real máximo) — date-lists caem pro LLM/conversa; palpites altos plausíveis ("Alemanha 7, Brasil 1") seguem ok. **Invariante de segurança confirmado por teste:** nenhum formato — palpite ou não — chega a "registrei/Combinado" sem **preview + confirmação sim/não + match de time real**; falsos-positivos no máximo geram um preview que o team-match rejeita. **+8 testes.** **1144 tests**, simulador 143. Sem migration. |
| **3.51.0** | **2026-06-29** | **BLINDAGEM robusta de palpite — fecha a classe inteira do bug (continuação do caso 29/06).** Em vez de só caçar regex, duas camadas: **(1) +2 formatos determinísticos** — `Time1 N, Time2 M` ("Brasil 2, Marrocos 1") e `Time1 N e Time2 M` ("Brasil 2 e Marrocos 1") via `PALPITE_LADOS_VIRGULA_E_REGEX` (a vírgula exige dígito ANTES → não quebra lista real "A 2x1 B, C 1x0 D", que segue pro splitter). **(2) Rede de segurança via LLM** — novo `pareceTentativaDePalpite` (heurística de alta precisão: frase curta, não-pergunta, com **2+ números de placar** + **2+ nomes prováveis de time**) dispara o **extrator LLM contra os jogos OFICIAIS da rodada aberta MESMO fora da janela de "próximos jogos"**. Como o extractor só age se casar times REAIS e SEMPRE passa pelo preview + confirmação (`tentarPalpiteLivreViaLLM`, contrato testado: nunca registra direto), qualquer formato futuro inesperado é entendido com segurança — e se o LLM não casar, segue o fluxo normal (sem forçar nada nem simular registro). Mensagem de "formato não entendido" atualizada com os novos exemplos. **+14 testes** (8 do detector + 3 do regex vírgula/"e" + 3 do gols-pos-time já existentes), **simulador 143**. **1136 tests.** Sem migration. |
| **3.50.0** | **2026-06-29** | **Fix GRAVE: palpite "Time1 N x Time2 N" não registrava (caso real 29/06).** Usuário mandou **"Alemanha 2 x Paraguai 3"** (gol DEPOIS de cada time, separador entre os lados) → o parser não reconhecia (caía em TEXTO_LIVRE), e o **"Sim"** que ele mandou em seguida virava `CONCORDANCIA_CASUAL` → **"🙌 Combinado!"**, dando a falsa impressão de que o palpite foi anotado — **mas não foi**. Era um formato natural não coberto pelos 4 regex existentes (canônico `Time NxN Time`, invertido `NxN Time x Time`, gols-separados `N Time x N Time`). Novo **`PALPITE_GOLS_POS_TIME_REGEX`** (`^(.+?)\s+(\d+)\s+[xX×]\s+(.+?)\s+(\d+)$`) como 5º formato — roda por ÚLTIMO (é o mais genérico) com os mesmos guards anti-lixo do gols-separados (não começa com dígito, não é stopword semântica, sem placar embutido). Agora o palpite cai no pipeline canônico de **preview + confirmação sim/não** e registra de verdade. Cobre multi-palavra ("Brasil 2 x Coreia do Sul 1"), `×` unicode e empate. **+5 testes unit + 1 no simulador (142)**. **1122 tests.** Sem migration. |
| **3.49.0** | **2026-06-29** | **Fix do "bom dia" DUPLICADO (caso real 29/06).** O aviso diário "bom dia boleiros" listava cada confronto DUAS vezes pra quem está em 2 bolões (Brasil x Japão ×2, etc.). Causa: `send-bom-dia.job` agregava os jogos só pela chave `waId` (telefone) e dava `push` da lista de CADA bolão — como cada bolão tem a sua própria rodada R32 com os mesmos confrontos, o jogo entrava 1× por bolão. **NÃO era duplicação no banco** (confirmado: "próximos jogos" mostrava 16/16 por bolão = 1 rodada R32 por bolão; palpites intactos — o fluxo multi-bolão preenche todos). Correção (só exibição): dedup por confronto (`apiJogoId`, fallback times+horário); o ✅ só aparece se a pessoa palpitou em TODOS os bolões com aquele jogo (senão ⚪, pra não esconder pendência num dos bolões). Prevenção: `seed-mata-mata.ts` agora roda o sync sob o lock `fetch-results` — serializa com o tick automático e fecha a corrida que (em tese) poderia criar rodada duplicada de verdade. Índice único `(bolaoId, fase)` avaliado e NÃO aplicado (migration de unique pode abortar o deploy se houver qualquer duplicata; o lock já fecha a corrida). **1117 tests** (+3 de dedup cross-bolão). Sem migration. |
| **3.48.0** | **2026-06-28** | **"Meus palpites" mostra e VALIDA o classificado do mata-mata + transparência na edição.** Caso real (Humberto 28/06): editou um palpite de empate (1x1, África crava) pra decisivo (1x2 Canadá) e o bot só disse "atualizado" — sem dizer o que virou o classificado. (1) **"meus palpites"** agora mostra o "quem passa" pra TODO jogo de mata-mata: empate→a crava (ou "falta escolher"), decisivo→o vencedor PELO PLACAR (ignora `classificadoPalpite` órfão de quando era empate). Em jogo ENCERRADO, **valida** contra quem passou de verdade: "✅ você acertou" / "você achou X, passou Y ❌". `ladoClassificadoImplicito`/`linhaClassificadoMeusPalpites` em `palpite-render.ts` (testáveis). (2) **Edição** de palpite decisivo de mata-mata anexa "🎯 Quem passa agora: *X* (pelo placar)". Placar oficial+pontos+bônus+pênaltis pra jogo encerrado já vinham do `montarStatusResultado`. Confirmado que `registrarPalpiteJogo` já zera o `classificadoPalpite` no decisivo (v3.45.0) — sem bug de scoring; o que faltava era o DISPLAY. **1114 tests** (+9), simulador 141/141. Sem migration. |
| **3.47.0** | **2026-06-28** | **Fix CRÍTICO do anti-loop: resposta repetida em fluxo não é mais silenciada.** Caso real (Andre 28/06): respondeu "2" pra um empate da fila de classificado e "2" DE NOVO pro próximo — a camada 4 do anti-loop (`verificarAntiLoop`, motivo `repetida`) silenciava a 2ª mensagem idêntica em <60s, então a fila travava e o user só conseguia confirmar 1 dos N classificados (depois a sessão expirava). Correção AMPLA: a camada "repetida" agora **só** vale em IDLE — quando o user está num fluxo interativo (state != IDLE), repetir a mesma resposta ("2"/"2", "sim"/"sim", "1"/"1") é legítimo e passa. O `cap_60s` (8 msgs/60s) segue barrando loop real de auto-reply. Sessão carregada antes do anti-loop pra saber o estado. Cobre TODOS os fluxos de confirmação/fila, não só o classificado. **1106 tests** (+4 no anti-loop), simulador 141/141. Sem migration. |
| **3.46.0** | **2026-06-28** | **Transparência do classificado multi-bolão.** Quando o palpite de empate foi aplicado em N bolões, o classificado (quem passa) vale pros N de uma vez — igual ao placar (que já é o mesmo nos N). Agora isso fica EXPLÍCITO: a pergunta diz "_(vale pros N bolões)_" e a confirmação usa a contagem real do `updateMany` ("anotado: X passa em *N* bolões"). Verificado que a gravação já cobria todos os bolões corretamente (via `rodadaId IN [...]` + nome do jogo) — sem bug. Diferenciar por bolão continua fora do atalho multi-bolão (é o mesmo palpite nos N por design; pra cravar diferente, palpitar por bolão). Sem mudança de schema/fluxo. **1103 tests, simulador 141/141.** |
| **3.45.0** | **2026-06-28** | **Auditoria do empate→classificado: helper único + EDITAR palpite + fetch-results destravado.** (1) **Bug de produção (fetch-results travado):** `fetchCalendar` da FIFA sem timeout pendurava o tick inteiro (lock preso, placar parado); + early-return quando não havia jogo "em andamento" fazia o sync NUNCA rodar com os grupos encerrados (R32 não abria). → `AbortSignal.timeout(12s)` + sync roda sempre. (2) **Auditoria de TODOS os caminhos de registro de palpite:** o fluxo empate→"quem se classifica?" estava duplicado e faltava em vários. Centralizado em `talvezPerguntarClassificadoMataMata` (fonte única) e ligado em: edição de palpite (3 caminhos — antes editar pra empate não perguntava), `handleConfirmandoPalpiteMultiBolao` (1 palpite→N bolões, bug Jeniffer 28/06), `handlePalpitando`. (3) **`classificadoPalpite` órfão:** `registrarPalpiteJogo` agora ZERA o campo quando o placar vira decisivo (antes "meus palpites" mostrava "você acha que X passa" num jogo que não era mais empate). (4) **Comando de versão** (dono manda "versão"/"#versao") + log de boot pra conferir produção. Auditoria confirmou OK: displays, jobs, recálculo, totais com bônus, apagar palpite. **1103 tests, simulador 141/141.** Sem migration. |
| **3.44.0** | **2026-06-28** | **Robustez de conversa do mata-mata + classificado no mesmo palpite do empate.** (1) **Classificado inline:** num empate dá pra dizer quem passa na MESMA mensagem ("Brasil 1x1 Japão e o Brasil passa"); `PalpiteInline` ganhou `classificado?` (parser: `separarClassificadoInline`/`resolverLadoClassificado` — corta o rabicho antes de parsear o placar [fim da poluição do nome] e resolve o lado pelos nomes; reanexa rabicho isolado por vírgula ao empate anterior; cobre e/mas/com/vírgula/parênteses, "quem passa é X", "classifico X", "vai o X", "X avança/se classifica"). O **preview de confirmação** mostra "↳ classificado: *X* passa" → user confirma placar + quem passa juntos ANTES de registrar (`registrarClassificadosInline` grava direto e só pergunta o que faltou; correção de LADO quando o resolver inverte a ordem dos times). Decisivo ignora o hint; ambíguo → pergunta. (2) **Guard anti-sequestro:** um palpite REAL não vira mais intent de dúvida do mata-mata ("…nos pênaltis" não cai mais em INFO_PENALTI perdendo o palpite) via `pareceTimeLimpo`. (3) **Roteamento (bateria de QA, 109 casos):** "jogos de hoje"/"horário dos jogos de hoje" → JOGOS_HOJE (era PERGUNTA_GERAL); "tabela" → RANKING; conjugações/typos de pênalti/empate/crava/ranking/bracket; "como funciona o mata-mata"/"mata-mata" → INFO_O_QUE_MUDA. (4) **Classificador LLM:** exceção do mata-mata na REGRA-CHAVE (time + adversário/horário/avançou → ADVERSARIO_TIME/HORARIO_JOGO/VER_CHAVE, não PERGUNTA_GERAL). (5) **Escape gracioso** do `CONFIRMANDO_CLASSIFICADO_MATAMATA`: comando forte ("ranking") no meio não trava nem perde dado — avisa que o placar está salvo e processa. Regras/`KNOWLEDGE_PRODUTO`/`infoEmpateMataMata` atualizados. **1101 tests, simulador 141/141.** Sem migration. |
| **3.43.0** | **2026-06-27** | **Sync AUTOMÁTICO dos confrontos do mata-mata pela API da FIFA — fim do seed manual.** Descoberto que a `api.fifa.com/api/v3/calendar/matches` (que o bot já consome pros placares) traz TODO o mata-mata: `MatchNumber` (73–104 = nossa numeração), `StageName` (=nossas fases 1:1), `Date` em UTC, times (`IdCountry`→PT via teams.json, 48/48 batem) e `Winner` (`IdTeam`) que resolve o classificado **inclusive nos pênaltis**. A chave do `bracket-2026.ts` foi CONFIRMADA pelo payload (os PlaceHolders W73/RU101/… batem) → TODOs "CONFERIR FIFA" removidos. **Novos:** `fifa.fetcher.parseFixturesMataMata`/`buscarFixturesMataMata`; `mata-mata.sync.service` (por bolão ativo: garante as 6 rodadas, upsert dos 32 jogos — preenche/corrige os times até a rodada abrir e depois TRAVA, espelha data/placar/classificado, reseta cálculo no que mudou, abre a rodada com ≥1 confronto real; idempotente). O `fetch-results` roda o sync a cada tick (fonte da verdade do mata-mata — o adapter de resultados só casava GRUPOS por código de país) + recalcula; o `advance-bracket` e o `#CLASSIFICADO` viram fallback. `seed-mata-mata` virou CLI fino sobre o sync (`--dry-run` mostra os jogos da FIFA). Verificado contra a API ao vivo. **1087 tests, simulador 141/141.** Sem migration. |
| **3.42.0** | **2026-06-27** | **Hardening do mata-mata — adaptar TODOS os comandos pra fase eliminatória + antecipar bugs.** Auditoria revelou que o bot assumia "1 rodada por bolão"; o mata-mata cria 6 rodadas que podem ficar ABERTAS ao mesmo tempo, com jogos placeholder ("Vencedor 73") até o advance. **P0:** `mostrarProximosJogos` agrega TODAS as rodadas ABERTA + filtra placeholder + mostra a fase; `iniciarConfirmacaoPalpites` extrai contra a união dos jogos abertos e registra cada palpite na rodada do seu jogo (fim do "não entendi" entre R32/oitavas); `fatos-vivos` ignora placeholder (LLM não cita "Vencedor 73"); **stall de pênaltis** resolvido (fifa.fetcher infere do placar de pênaltis quando o payload traz; novo comando admin `#CLASSIFICADO <apiJogoId> <lado> [PENALTIS]` grava em todos os bolões + recalcula + avança). **P1 (display):** fase em "meus palpites"/ranking/progresso (corrige "Fase de Grupos" hardcoded); "(nos pênaltis)" em meus-palpites/placar; classificado cravado em meus-palpites/revelação; bônus na estatística; nota de mata-mata no preview do palpite. **P2 (guards):** `timeCorresponde` rejeita placeholder; reveal/lembrete/bom-dia filtram placeholder; `escreverSlot` do advance vira `updateMany` atômico. **1087 tests, simulador 141/141.** Sem migration. |
| **3.41.0** | **2026-06-27** | **Fase MATA-MATA da Copa 2026 (16-avos → final + 3º lugar).** Migration 100% aditiva (enums `FaseTorneio`/`LadoJogo`; `fase` em Rodada/Jogo; `classificadoLado`/`decididoNosPenaltis`/`proximoJogoApiId`/`proximoSlot` em Jogo; `classificadoPalpite`/`bonusObtido` em PalpiteJogo) — grupos intactos. **Pontuação por fase** (`TABELA_PONTOS`/`BONUS_CLASSIFICADO`, fonte única; GRUPOS e R32 = `PONTUACAO_PADRAO`) + **bônus de classificado aditivo** (`pontuarJogoMataMata`): placar vale 90'+prorrogação, **pênalti nunca entra no placar**; classificado inferido no decisivo, perguntado no empate (novo estado FSM `CONFIRMANDO_CLASSIFICADO_MATAMATA`, fila de pendências). **Chave determinística** `bracket-2026.ts` (R32→oitavas confirmados; quartas→final com CONFERIR FIFA) + mapa Sede→IANA. **Seed** `seed-mata-mata.ts` (dados à noite só preenchendo o bloco; idempotente; `--dry-run`; portão por times reais). **advance-bracket** propaga vencedor→próximo jogo e perdedor das semis→3º lugar (idempotente). **Submenu de regras** (completas/mata-mata) + **11 intents novos** de dúvida/chave (INFO_PRORROGACAO, INFO_PENALTI, INFO_EMPATE_MATAMATA, INFO_PONTOS_MATAMATA, INFO_BONUS_CLASSIFICADO, INFO_CRAVA_EMPATE, INFO_RANKING_CONTINUA, INFO_O_QUE_MUDA, ADVERSARIO_TIME, HORARIO_JOGO, VER_CHAVE) + BASE_CONTEXT/KNOWLEDGE_PRODUTO atualizados. Ver seção **23**. **1071 tests, simulador 139/139.** Migration `20260627120000_add_mata_mata` (aplicar com `migrate deploy`). |
| **3.40.0** | **2026-06-22** | **Fechando 6 buracos da revisão diária (mensagens não-entendidas), sem novos intents.** (1) **"como posso fazer um bolão da minha família?"** caía em DESCONHECIDO — `CRIAR_BOLAO_PATTERNS` exigia imperativo no início (quero/bora). Novos patterns pra pergunta com verbo de criação (`como (posso) criar/fazer/abrir … bolão`) + imperativo 3ª pessoa (`cria/faz/monta um bolão`); prompt do classifier atualizado. (2) **"refazer" sozinho** em IDLE não casava nada (só era tratado dentro dos estados CONFIRMANDO_*); novo pattern `^refazer/refaz$` → EDITAR_PALPITE. (3) **"3x0"** (placar sem time) caía em llm_fail; nova heurística `parecePalpiteSoPlacar` → bot pede o jogo. (4) **lista de confrontos SEM placar** ("Noruega x França\nSenegal x Iraque\n…", 6 jogos) caía em llm_fail; nova `pareceListaDeConfrontosSemPlacar` → bot pede o placar ecoando um exemplo. (5) **"Meus olhares"** (autocorretor de "meus palpites") → alias em `MEU_PALPITE_PATTERNS`. As 2 heurísticas novas entram no `handleIdle` ANTES do smart-fallback (read-only, sem alucinar registro). Não-regressão: "como funciona o bolão"→AJUDA, "refazer Brasil 2x1"/"refazer palpite"→EDITAR_PALPITE, "Brasil 3x0"→palpite incompleto, lote com dígitos→PALPITE_INLINE. **21 testes novos.** **1015 tests, simulador 129/129.** Sem migration. |
| **3.39.0** | **2026-06-22** | **Drill-down da estatística: listar os JOGOS de cada faixa de pontos (caso Humberto 22/06).** Depois da v3.38.0, ele perguntou "Quais jogos eu cravei?" e o bot caiu no handler genérico (`handleMeusPalpites` → "Quer ver palpites detalhados? sim/não") em vez de listar os jogos cravados — "quais ... cravei" não casava regex (ESTATISTICA exige "quantos/quantas") e ia pra LLM→MEU_PALPITE. **Nova intent read-only `JOGOS_POR_FAIXA`**: lista os jogos do user de uma faixa (10/7/5/3/0) com palpite + resultado real, via `getJogosPorFaixa`/`buscarPalpitesJogosPorFaixa` (reusa `getEstatisticaPontos` pra a régua de faixas no rodapé). Faixa detectada por `detectarFaixaEstatistica` (agora também pega "deu/deram/valeu N" sem a palavra "pontos"). Patterns exigem gatilho de LISTAGEM (quais/me mostra/ver) + faixa e vêm ANTES de ESTATISTICA_PONTOS → "quantas cravadas" (contagem) e "quais cravei" (lista) não colidem. Reusa o estado `ESCOLHENDO_BOLAO_ESTATISTICA` (ctx `estatisticaListarJogos`) e fatora `resolverBolaoEstatistica` (compartilhado com a estatística). Rodapé da ESTATISTICA agora cross-linka o drill-down. **20 testes novos** (parser 12 frases + não-regressão, `getJogosPorFaixa` mock prisma). **994 tests, simulador 125/125.** Sem migration. |
| **3.38.0** | **2026-06-22** | **Nova feature: estatística de pontos por FAIXA (caso Humberto 22/06).** Ele perguntou "Quantos jogos eu fiz 10ponto?" e "de todos meus palpites, quantos acertei o placar exato?" — o bot respondeu o `PONTOS_DETALHE` (lista jogo-a-jogo das últimas 48h), que **não conta** por faixa nem cobre o histórico todo. **Nova intent read-only `ESTATISTICA_PONTOS`**: conta os `pontosObtidos` por faixa (cravadas/10, 7, 5, 3, 0) sobre palpites JÁ calculados (jogo `FINALIZADO` + `Palpite.calculado=true`) via `getEstatisticaPontos` (faixas de `PONTUACAO_PADRAO`, sem hardcode). Mostra a quebra + total + posição; pergunta por faixa específica ganha destaque no topo ("🎯 Você cravou *2 vezes*!"). Resolve o bolão espelhando o RANKING (padrão → 1 direto → estado `ESCOLHENDO_BOLAO_ESTATISTICA`). **Precedência ANTES de PONTOS_DETALHE/MEUS_PONTOS**; patterns exigem menção a faixa/estatística → "quantos pontos fiz ontem"→PONTOS_DETALHE e "meus pontos"→MEUS_PONTOS seguem intactos (testes de não-regressão). Whitelist do classifier + prompt + teste anti-drift cobrem a long tail. **Read-only**: não registra nada nem chama o responder conversacional → sem risco de alucinar "anotei palpite". **17 testes novos** (parser 18 frases + não-regressão, `getEstatisticaPontos` mock prisma). **974 tests, simulador 121/121.** Sem migration. |
...
## 23. Mata-mata (Copa 2026)

Suporte à fase eliminatória: **16-avos (R32) → oitavas → quartas → semi → 3º lugar → final**.
Implementado como camada **aditiva** sobre a fase de grupos — grupos não regridem.

### 23.1 Schema (migration `20260627120000_add_mata_mata`, 100% aditiva)
- Enums: `FaseTorneio { GRUPOS R32 OITAVAS QUARTAS SEMI TERCEIRO FINAL }`, `LadoJogo { CASA VISITANTE }`.
- `Rodada.fase` (default GRUPOS); `Jogo.fase` + `classificadoLado?` + `decididoNosPenaltis?` +
  `proximoJogoApiId?` + `proximoSlot?`; `PalpiteJogo.classificadoPalpite?` + `bonusObtido` (default 0).
- `bonusObtido` é coluna **separada** de `pontosObtidos` de propósito: preserva a bucketização por
  faixa de `getEstatisticaPontos`/`getJogosPorFaixa` (no mata-mata as faixas são por fase).

### 23.2 Pontuação (`src/modules/ranking/`)
- `TABELA_PONTOS: Record<FaseTorneio, PontuacaoConfig>` e `BONUS_CLASSIFICADO: Record<FaseTorneio, number>`
  em `ranking.types.ts` — **fonte única**. `GRUPOS` e `R32` referenciam `PONTUACAO_PADRAO`.
  Placar exato por fase: R32 10 · oitavas 12 · quartas 15 · semi 18 · 3º 12 · final 22.
  Bônus: R32/oitavas +3 · quartas +4 · semi +5 · 3º +3 · final +6.
- `pontuarJogoMataMata(args) → { placar, bonus }` (`pontuacao.calc.ts`): placar via `calcularPontos`
  com a config da fase; classificado **inferido** do vencedor no decisivo, **explícito** no empate.
  **Placar vale 90'+prorrogação; pênalti nunca entra.** Errar o classificado nunca tira a crava.
- `calcularPontuacaoRodada` ramifica por `jogo.fase` (GRUPOS = caminho idêntico ao de hoje).

### 23.3 Chave (`src/data/bracket-2026.ts`)
- `BRACKET_2026` (apiJogoId → `{ vencedor?, perdedor? }`): R32→oitavas e oitavas confirmados;
  quartas→final como padrão **CONFERIR FIFA**. `JOGOS_MATA_MATA` (descritores 73–104),
  `apiIdMataMata`, `rotuloAlimentador`/`ehTimePlaceholder` (placeholder "Vencedor 73"),
  `faseLabel`, `SEDE_PARA_IANA`/`ianaDaSede` (DST-aware via IANA).
- IDs: `WC2026_R32_73..88`, `WC2026_OIT_89..96`, `WC2026_QUA_97..100`, `WC2026_SEMI_101..102`,
  `WC2026_TER_103`, `WC2026_FIN_104`.

### 23.4 Sync automático pela FIFA (`mata-mata.sync.service`, v3.43.0)
- **Fonte da verdade dos confrontos = a api.fifa.com.** `buscarFixturesMataMata()` (em `fifa.fetcher`)
  lê o mesmo `calendar/matches` dos placares e devolve os 32 jogos de mata-mata: número→`apiJogoId`,
  `StageName`→fase, `Date`→kickoff UTC, `IdCountry`→time PT (ou null), placar (90'+prorrog) e
  classificado do `Winner` (`IdTeam`, resolve pênaltis).
- `sincronizarMataMata(prisma, fixtures)`: por bolão ATIVO garante as 6 rodadas, faz upsert dos jogos
  — **preenche/corrige os times enquanto a rodada está FECHADA e TRAVA quando ela abre** (protege
  palpites; nunca rebaixa real→placeholder); espelha data/placar/classificado; reseta `calculado` no
  que mudou; **abre a rodada quando tem ≥1 confronto real**. Idempotente (só escreve no que mudou).
- Roda no tick do `fetch-results` (a cada 5min) + recalcula as rodadas/ranking afetados.
- `scripts/seed-mata-mata.ts` é CLI sob demanda: `--dry-run` mostra os jogos da FIFA (não precisa de
  DATABASE_URL); sem a flag, sincroniza na hora.

### 23.5 Fetch + advance (`src/modules/resultado/`, `src/jobs/advance-bracket.job.ts`)
- `fetch-results` captura `classificadoLado`/`decididoNosPenaltis` (placar = 90'+prorrogação) e, no
  fim do tick, roda `advanceBracketInterno` (mesmo lock).
- `advance-bracket` grava o vencedor no slot do próximo jogo (e o perdedor das semis no 3º lugar),
  abre a rodada quando os dois lados ficam reais. **Idempotente** — só escreve slot placeholder.

### 23.6 UX / FSM / intents
- Estado `CONFIRMANDO_CLASSIFICADO_MATAMATA` (protegido contra FSM-escape, na blacklist de código):
  após confirmar o palpite, cada **empate** cravado dispara a pergunta "quem passa nos pênaltis?",
  em fila (cobre multi-palpite e multi-bolão). Grava `classificadoPalpite` via
  `palpiteService.registrarClassificadoPalpite`.
- Estado `ESCOLHENDO_TIPO_REGRAS`: `REGRAS` pergunta "completas ou mata-mata?"; `regrasCompletas()`
  e `regrasMataMata()` em `regras.text.ts`.
- 11 intents novos (ver seção 7 / `commands.md`): 8 dúvidas de resposta fixa (`mata-mata.respostas.ts`)
  + `ADVERSARIO_TIME`/`HORARIO_JOGO`/`VER_CHAVE` (leem a chave via `bracket.service.ts`; jogo indefinido
  → reportam a dependência, nunca inventam). `BASE_CONTEXT`/`KNOWLEDGE_PRODUTO` cobrem a long tail.

### 23.8 Multi-rodada e comando admin (v3.42.0)
- Várias rodadas de mata-mata podem ficar ABERTA ao mesmo tempo. `mostrarProximosJogos` e
  `iniciarConfirmacaoPalpites` agregam os jogos de TODAS as rodadas ABERTA do bolão e filtram
  jogos placeholder (`ehTimePlaceholder`); cada palpite é registrado na rodada do seu jogo
  (`palpitesParaConfirmar[].rodadaId`).
- **Placeholder NUNCA vaza** pro usuário: filtrado em próximos-jogos, fatos-vivos (LLM),
  progresso, reveal, lembrete-30min e bom-dia; `timeCorresponde` rejeita nomes placeholder.
- **Pênaltis:** o adapter FIFA infere o classificado do placar de pênaltis quando o payload traz;
  fallback é o comando admin (só dono) `#CLASSIFICADO <apiJogoId> <CASA|VISITANTE|nome> [PENALTIS]`
  (`src/whatsapp/admin-classificado.ts`, interceptado após o broadcast) — grava em todos os
  bolões, reseta `calculado`, recalcula e dispara o `advance-bracket`.

### 23.7 Invariantes (não podem regredir)
Migration aditiva; grupos intactos; GRUPOS=`PONTUACAO_PADRAO`; placar 90'+prorrog (pênalti fora);
bônus aditivo em coluna separada (crava nunca perdida); seed/advance idempotentes; kickoff em UTC,
exibido em America/Sao_Paulo; FSM de classificado protegido; ranking cumulativo (não zera).

---

## 24. Canal Telegram (multi-canal, v3.59.0)

### 24.1 Princípio
O bot fala WhatsApp **e** Telegram com a **mesma lógica**. O `command.router`
(295KB de features) é agnóstico de canal — identifica tudo por `waId` e responde
via `sendText({ to })`. O Telegram foi adicionado **só na fronteira de I/O**
(`src/messaging/`), então **toda** funcionalidade existente (palpites, ranking,
mata-mata, regras, LLM, comandos admin `#…` do dono) funciona no Telegram sem
reimplementação. Contexto: número do WhatsApp com bloqueio de SAÍDA em plena
Copa (01/07) — o Telegram não tem esse risco (Bot API é oficial e feita pra bots).

### 24.2 Fluxo de dados
```
ENTRADA  Telegram → polling OU webhook → telegram.inbound
           ├─ sem vínculo → onboarding (pede nº WhatsApp → vincula/cria)
           └─ com vínculo → handleIncomingMessage({ waId: usuario.whatsappId, … })

SAÍDA    sendText({ to: waId })  (os 324+ callsites de sempre)
           └─ channel-router: Usuario.telegramId + canalPreferido + flags
                ├─ 'whatsapp' → Evolution (igual hoje)
                ├─ 'telegram' → telegram.client (HTML, quebra 4096, retry 429)
                └─ 'drop'     → loga e não envia (sem canal viável)
```
Endereço `tg:<chatId>` força Telegram (usado no onboarding, antes do vínculo).
Dry-run: o capture acontece ANTES do roteamento — simulador/testes enxergam
todas as mensagens independente de canal.

### 24.3 Identidade / recuperação de pontuação
No 1º contato (`/start`), o bot pede o **número de WhatsApp** da pessoa,
normaliza (`normalizarNumeroBR`) e casa contra `Usuario.whatsappId` com
variantes (com/sem 9º dígito BR, com/sem sufixo `@s.whatsapp.net`). Achou →
confirma o nome ("Achei: *Fulano* — é você?") e grava `telegramId` +
`canalPreferido='telegram'` → **histórico e pontos aparecem sozinhos** (tudo
FKa `usuario.id`). Não achou → oferece criar cadastro novo. Número já vinculado
a OUTRO chat → recusa (não rouba vínculo). FSM própria em Redis
(`tg_onboarding:<chatId>`, TTL 30min), separada da FSM de conversa.

### 24.4 Transportes
- **polling** (default): loop `getUpdates` long-poll (~50s); 1 loop pro bot
  inteiro (o lote traz TODAS as conversas — 200 conversas ≠ 200 conexões).
  Zero infra pública; backoff exponencial em erro (1s→60s).
- **webhook**: `POST /webhook/telegram` (rota registrada quando
  `ENABLE_TELEGRAM=true`), validação por `X-Telegram-Bot-Api-Secret-Token`,
  200 imediato + processamento async. `setWebhook` no boot. Exige `APP_URL`
  público HTTPS. Dedup por `update_id` em Redis cobre re-entregas.

### 24.5 Formatação (telegram.format.ts)
Todo texto do bot usa o dialeto do WhatsApp. O conversor gera **HTML**
(`*b*`→`<b>`, `_i_`→`<i>`, `~s~`→`<s>`, `` `c` ``→`<code>`, ```pre```→`<pre>`),
escapa `<>&`, respeita as regras de par do WhatsApp (mesma linha, sem espaço
interno) e **nunca lança** — marcador solto fica literal. Se o Telegram ainda
rejeitar (400), re-envia como **texto puro** (fallback: melhor sem negrito do
que sem resposta). Mensagens >4096 chars são quebradas em `\n`.

### 24.6 Cenários de flag
| ENABLE_WHATSAPP | ENABLE_TELEGRAM | Comportamento |
|---|---|---|
| true | false | **Default — idêntico a hoje** (rota WhatsApp sem tocar no banco) |
| true | true | Convive: linkados com `canalPreferido='telegram'` recebem no Telegram; resto no WhatsApp |
| false | true | **Migração**: tudo via Telegram; sem vínculo → drop logado |
| false | false | Warn no boot; nada é enviado |

### 24.7 Invariantes
- Com flags default, NENHUM comportamento muda (rota curta sem query).
- `command.router`/jobs continuam sem saber que o Telegram existe.
- Vínculo nunca é sobrescrito por outro chat sem desvincular antes.
- `markAsRead` é no-op pra endereços `tg:`.
- Falha no boot do Telegram NUNCA derruba o canal WhatsApp (try/catch).

---

## 25. Histórico de conversas + auditoria de palpite (v3.60.0)

### 25.1 Motivação
Auditoria: no WhatsApp o dono via as conversas no próprio app (o bot era um
número real); bot do Telegram não tem cliente. E disputa de palpite ("eu
mandei 2x1, o bot registrou errado") precisa de prova. Duas peças:

### 25.2 `mensagens_conversa` — histórico completo
- Grava TODA mensagem (ENTRADA e SAIDA), nos 2 canais, nos 2 chokepoints:
  topo de `handleIncomingMessage` (+ entrada do onboarding Telegram, que
  roda antes do router) e `sendText`/`sendImage` (pós-roteamento, com o
  canal REAL). Fire-and-forget (`void`): banco fora → `console.warn`,
  conversa segue. Pulado em `DRY_RUN_WHATSAPP` (testes/sim não sujam banco).
- Colunas: `usuarioId?` (resolvido por variantes do waId, fora do caminho
  crítico), `waId` cru, `canal`, `direcao`, `texto` (truncado 2000),
  `messageId?`, `criadoEm`.
- Retenção LGPD: job mensal (`limpar-mensagens-antigas`) deleta o que passar
  de `CONVERSA_RETENCAO_DIAS` (default 180).

### 25.3 `palpites_auditoria` — trilha de auditoria (event log)
- Evento imutável por ação: `REGISTRADO` / `EDITADO` (placar antes→depois) /
  `APAGADO` / `CLASSIFICADO` (quem passa no mata-mata), com `textoOriginal`
  (a mensagem CRUA que gerou a ação) e `canal`. NÃO expira.
- Hooks nos pontos únicos de escrita: `registrarPalpiteEmRodada` (funil de
  todos os fluxos de registro/edição), `apagarPalpiteJogo`,
  `registrarClassificadoPalpite` (1 evento por jogo atingido).
- O texto cru chega via `auditoria-contexto.ts`: o router seta
  (usuarioId → {texto, canal}) no início de cada mensagem; o service lê.
  Em memória (TTL 10min) — o dado só precisa viver durante o processamento
  da própria mensagem.

### 25.4 Comandos de dono (`admin-conversas.ts`)
Interceptados no topo do pipeline (padrão broadcast — só dono, não-dono não
fica sabendo que existem):
- `#CONVERSASGLOBAL [N]` — últimas N mensagens gerais (default 20, cap 100),
  com o nome de quem mandou.
- `#CONVERSAS <número|nome> [N]` — conversa de UMA pessoa (busca por número
  com variantes de 9º dígito/JID, ou nome fuzzy; vários matches → pede refino).
- `#AUDITORIA <número|nome> [N]` — extrato de palpites (cap 50):
  `[01/07 14:33] *Brasil x França*: EDITOU 2x1 → 3x1 via telegram
     ↳ msg: "corrigir Brasil 3x1 França"`.

### 25.5 Invariantes
- Logging NUNCA bloqueia nem derruba conversa (fire-and-forget + catch).
- Zero latência extra no caminho crítico (lookups rodam dentro do void).
- Auditoria de palpite é imutável e não expira; conversas expiram por env.
- Consulta só pelo(s) dono(s) (`OWNER_WHATSAPP_IDS`).
