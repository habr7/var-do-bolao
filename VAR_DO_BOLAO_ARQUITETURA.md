...
**Versão do documento:** 3.3.0
**Última atualização:** 2026-05-18 (nova intent PERGUNTA_GERAL_FUTEBOL + LLM responder reescrito pra responder perguntas gerais)
...
| **3.3.0** | **2026-05-18** | **Nova intent `PERGUNTA_GERAL_FUTEBOL` + LLM responder reescrito.** Bug reportado da VPS: usuário perguntava "Quais próximos jogos da Inglaterra?" e "Qual canal posso assistir o Brasil hoje?" — bot respondia "não faz parte de nenhum bolão" / "bolão não encontrado". Causa tripla: (1) regex `\bproximos? jogos?\b` matchava substring de qualquer pergunta; (2) classifier LLM não tinha guidance pra perguntas gerais; (3) `responderConversacional` proibia explicitamente "inventar dados" mas não autorizava respostas gerais. **Fix:** (a) Nova intent `PERGUNTA_GERAL_FUTEBOL` com patterns dedicados (qual canal/onde assistir/que horas joga/quem ganhou/grupo da copa/jogos da [time]). (b) `PROXIMOS_JOGOS_PATTERNS` ganhou negative lookahead `(?!\s+d[aoe]\s+\w)` — não matcha "próximos jogos **da** Inglaterra". (c) Novo handler `handlePerguntaGeralFutebol` chama `responderConversacional` direto. (d) Prompt do responder **autorizado** a responder usando conhecimento próprio (times, copas, regras, transmissão genérica com disclaimer); mantém proibição de inventar dados do bolão do user (palpites, ranking, IDs). (e) `INTENT_CLASSIFIER_PROMPT` ganhou regra-chave: pergunta com TIME/PAÍS/CANAL/JOGO específico → `PERGUNTA_GERAL_FUTEBOL`. **397 tests (era 384), 116 cenários (era 106).** (este documento) |
