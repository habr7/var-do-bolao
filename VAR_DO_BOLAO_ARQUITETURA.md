...
**Versão do documento:** 3.2.1
**Última atualização:** 2026-05-18 (hotfix 4-bug Humberto: pontuação/ajuda/CRIAR_BOLAO espúrio/FSM escape em criação)
...
| **3.2.1** | **2026-05-18** | **Hotfix 4 bugs Humberto:** (1) "Pontuação" sozinho ia pra RANKING("pontuacao") — MEUS_PONTOS_PATTERNS ampliado pra cobrir `^pontuacao$`, `^pontos?$`, `^score`, `^quanto pontuei`. (2) "Ajuda" mostrava texto legado com `!comandos` da v1 — `formatAjuda` reescrito pra UX atual (linguagem natural, todos os comandos novos: cordialidade, editar/apagar palpite, bolão padrão, etc). (3) "Bolao teste oficial" sozinho era classificado como CRIAR_BOLAO pelo LLM e criava bolão duplicado — agora o handler de CRIAR_BOLAO chama `tentarOferecerMenuContextualPorNomeBolao` que fuzzy-matcha com bolões que o user participa e oferece menu contextual; prompt LLM tweaked pra exigir verbo de ação. (4) "Proximos jogos" no estado CRIANDO_BOLAO_NOME virava nome do bolão — novo helper `tentarFsmEscapeCriandoBolao` detecta intent forte em `CRIANDO_BOLAO_*` e auto-cancela criação + reprocessa. **384 tests (era 377), 106 cenários (era 102).** (este documento) |
