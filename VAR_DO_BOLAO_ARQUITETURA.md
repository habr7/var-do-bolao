...
**Versão do documento:** 3.19.0
**Última atualização:** 2026-06-11 (BUG CRÍTICO Natane 11/06: `tentarPalpiteLivreViaLLM` registrava palpites SEM mostrar preview e respondia "Registrei N palpites!" mesmo com LLM podendo ter alucinado. Refatorado pra delegar ao pipeline canônico de confirmação. Novo regex `PALPITE_GOLS_SEPARADOS_REGEX` cobre formato "N Time X N Time" sem LLM. Teste de contrato impede regressão. Script `auditar-recuperar-palpite.ts` pra recuperação manual)
